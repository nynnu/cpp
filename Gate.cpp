...
Gate::Gate(int maxY, int maxX, int spawnInterval)
    : maxY(maxY), maxX(maxX), spawnInterval(spawnInterval), isActive(false), snakeInGate(false) {
    startTime = time(NULL);
}

void Gate::initialize(Map& map, Board& board) {
...
std::pair<int, int> Gate::getNextPosition(const SnakePiece& head, DIRECTION& newDir) {
...
void Gate::findWallPositions(Map& map) {
    wallPositions.clear();
...
void Gate::findEmptyPositions(Map& map) {
...
void Gate::placeGates(Board& board) {
...
bool Gate::isWall(const std::pair<int, int>& position) {
...
DIRECTION Gate::getNewDirection(DIRECTION currentDir, std::pair<int, int> newPos, std::pair<int, int> oldPos) {
